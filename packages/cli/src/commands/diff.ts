...
export function diffCommand(program: Command) {
  program
    ...
    .option("-f, --file <filename>", "Specify a different local file to compare (default: .env)")
    ...
    .action(async (projectName, options) => {
      ...
      if (fs.existsSync(envPath)) {
        ...
        console.log(chalk.yellow(`Local file ${localFileName} not found. Assuming local is empty.`));
      }

      const spinner = ora(`Fetching cloud variables for ${chalk.bold(finalProjectName)}...`).start();
      ...
      try {
        ...
        const cloudEnvConfig = cloudEnvContent ? dotenv.parse(cloudEnvContent) : {};
        
        const localKeys = Object.keys(localEnvConfig);
        const cloudKeys = Object.keys(cloudEnvConfig);
        const allKeys = Array.from(new Set([...localKeys, ...cloudKeys])).sort();
        ...
        for (const key of allKeys) {
          ...
          if (inLocal && !inCloud) {
            console.log(chalk.green(`+ Added locally:   ${key} = ${localEnvConfig[key]}`));
            hasDifferences = true;
          } else if (!inLocal && inCloud) {
            console.log(chalk.red(`- Removed locally: ${key} = ${cloudEnvConfig[key]}`));
            hasDifferences = true;
          } else if (inLocal && inCloud) {
        ...
        if (!hasDifferences) {
          console.log(chalk.green("✔ No differences found. Both files are identical."));
        }
