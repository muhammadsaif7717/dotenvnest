...
async function runCommand() {
  ...
  const fetchUrl = new URL(`/api/cli?project=${encodeURIComponent(projectName)}`, BASE_URL).toString();

  process.stdout.write(`\x1b[2m[DotEnvNest]\x1b[0m Fetching env for \x1b[36m${projectName}\x1b[0m… `);
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error(`\x1b[31mError ${res.status}:\x1b[0m ${data.error || 'Failed to fetch env.'}`);
      process.exit(1);
    }
    ...
    console.log(`\x1b[2m[DotEnvNest]\x1b[0m Injected \x1b[33m${Object.keys(envVars).length}\x1b[0m variable(s) → \x1b[36m${command}\x1b[0m`);

    const child = spawn(command, commandArgs, { env: finalEnv, stdio: 'inherit', shell: true });
    child.on('error', (err) => { console.error(`\x1b[31mFailed to start:\x1b[0m ${err.message}`); });
...
async function pullCommand() {
  ...
  try {
    ...
    if (!email) { console.error('\x1b[31mError:\x1b[0m Email is required.'); process.exit(1); }

    password = await promptPassword('  \x1b[2mPassword:\x1b[0m ');
    if (!password) { console.error('\x1b[31mError:\x1b[0m Password is required.'); process.exit(1); }
  } catch (e) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (res.status === 401) {
        ...
        console.error(`  \x1b[31m${data.error || `Project "${projectName}" not found.`}\x1b[0m`);
      } else if (res.status === 429) {
        console.error(`  \x1b[31mToo many attempts. Please wait before trying again.\x1b[0m`);
      } else {
        console.error(`  \x1b[31mError ${res.status}:\x1b[0m ${data.error || 'Unknown error.'}`);
      }
      process.exit(1);
    }
    ...
    console.log(`  \x1b[32m✓ Downloaded\x1b[0m \x1b[33m${keyCount}\x1b[0m variable(s) → \x1b[36m${outputFile}\x1b[0m`);
    console.log(`  \x1b[2mFile permissions set to 600 (owner read/write only)\x1b[0m\n`);
...
async function pushCommand() {
  ...
  console.log(`  \x1b[2mKeys:\x1b[0m    \x1b[33m${keyCount}\x1b[0m variable(s) found\n`);

  // ── Confirm unless --force ──────────────────────────────────────────────
  if (!force) {
    const confirm = await prompt(`  \x1b[33mPush "${inputFile}" → project "${projectName}"? [y/N]\x1b[0m `);
  ...
  try {
    ...
    const action = data.created ? '\x1b[32mCreated\x1b[0m' : '\x1b[34mUpdated\x1b[0m';
    console.log(`  ${action} project \x1b[36m${projectName}\x1b[0m with \x1b[33m${keyCount}\x1b[0m variable(s)`);
